...
export function IdeaPanel() {
  const { profile, company, updateCompany, fields } = useGame();
    ...
    <>
      <Panel title="The Idea Forge" hint="Name it before you build it">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border-2 border-ink/10 bg-white px-3.5 py-2.5">
            <p className="font-mono text-[11px] uppercase tracking-wider text-graphite">Founder</p>
            <p className="mt-0.5 font-display text-lg font-bold">
              {profile.firstName || 'You'}, age {profile.age}
            </p>
          </div>
          <Field
            label="Company name (change it any time)"
            value={company.name}

...
export function CommandPanel() {
  const { company, sales, revenue, profit, weeks, addWeek, artifacts, buildArtifact, xp, backing } =
  useGame();
    ...
    <>
      <Panel title="Company Dashboard" hint={`${company.name} · ${xp} XP`}>
        <div className="grid gap-3 sm:grid-cols-4">
          <Kpi label="Backed" value={`${backing.toLocaleString()}`} tone="#635BFF" />
          <Kpi label="Sales logged" value={String(sales.length)} tone="#2F5D8C" />
          <Kpi label="Product revenue" value={`${revenue.toLocaleString()}`} tone="#E0562A" />
          <Kpi
            label="Profit"
        ...
        <ul className="divide-y divide-ink/10 rounded-xl border-2 border-ink/10 bg-white">
          ...
          <li key={sale.id} className="flex items-center justify-between px-3.5 py-2.5 text-sm">
                <span>
                  ...
                  {sale.kind === 'backing' ?
              <span className="ml-2 rounded-full bg-[#635BFF]/10 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider text-[#635BFF]">
                      backer
                    </span> :
              null}
                </span>

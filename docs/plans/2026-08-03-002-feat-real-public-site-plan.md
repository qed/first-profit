...
- **Known bounded staleness:** a publish that proceeds after a parked flush (offline completion) leaves the page's first render behind until the outbox replays — possibly a later session. The success criterion's "within seconds" holds for the online path; the offline tail is stated in the room's not-live-yet state rather than hidden.

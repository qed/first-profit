  ...
  return (
    <div aria-labelledby="fp-sale-logger-title" className="pb-2">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div aria-label="Sale Logger progress" className="grid shrink-0 grid-cols-3 gap-1.5 rounded-[14px] border-2 border-build/20 bg-build/5 p-2.5 shadow-card">
          {[
            ["1", "Deliver", deliveryDone],
            ["2", "Feedback", feedbackDone],
            ["3", "Photo", proofDone],
          ].map(([number, label, done]) => (
            <div key={String(number)} className="min-w-[58px] text-center">
              <span className={`mx-auto flex h-8 w-8 items-center justify-center rounded-full border-2 font-display text-xs font-black ${done ? "border-verified bg-verified text-white" : "border-[hsl(25_34%_20%/0.14)] bg-white text-[hsl(25_20%_38%)]"}`}>{done ? <Check size={15} strokeWidth={3} aria-hidden /> : number}</span>
      ...
      <section className="mt-4 rounded-[14px] border-2 border-[hsl(25_34%_20%/0.14)] bg-white p-4 shadow-card" aria-labelledby="fp-sale-delivery">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-[150px_minmax(0,1fr)] lg:grid-cols-1">
